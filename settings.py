...
class Settings:
    def __init__(self):
        ...
        self.colors_of_life_intro = [(randint(1,255), randint(1,255), randint(1,255)) for x in range(15)]
        self.dimmer = 255
        for color in range(13):
            self.colors_of_life_intro.extend([randint(1,self.dimmer), randint(1,self.dimmer), randint(1,self.dimmer)])
            self.dimmer -= 17
